...
#include "cifer/innerprod/fullysec/mcfe.h"
#include "cifer/internal/dlog.h"
#include "cifer/internal/hash.h"
#include "cifer/sample/uniform.h"

//for clients
void cfe_mcfe_client_init(cfe_mcfe_client *c, size_t idx) {
...
void cfe_mcfe_client_free(cfe_mcfe_client *c) {
    mpz_clear(c->order);
    cfe_mat_free(&(c->share));
    cfe_vec_free(&(c->s));
}

void cfe_mcfe_set_share(cfe_mcfe_client *c, ECP_BN254 *pub_keys, size_t num_clients) {
    cfe_mat add;
...
//不变
void cfe_mcfe_encrypt(ECP_BN254 *cipher, cfe_mcfe_client *c, mpz_t x, char *label, size_t label_len) {
    ECP_BN254 h;
    BIG_256_56 tmp_big;
...
void cfe_mcfe_fe_key_part_init(cfe_vec *key_share) {
    cfe_vec_init(key_share, 2);
}

void cfe_mcfe_derive_fe_key_part(cfe_vec *fe_key_part, cfe_mcfe_client *c, cfe_vec *y) {
    ...
    for (size_t k = 0; k < 2; k++) {
        ...
        // ECP2_BN254_mul(&h, tmp_big);
        // ECP2_BN254_add(&(fe_key_part->vec[k]), &tmp_big);///

        cfe_vec_set_const(&(fe_key_part->vec[k]), &tmp);
    }
    mpz_clear(tmp);
}

cfe_error cfe_mcfe_decrypt(mpz_t res, ECP_BN254 *ciphers, cfe_vec *key_shares,
                            char *label, size_t label_len, cfe_vec *y, mpz_t bound) {
    cfe_vec keys_sum;
    cfe_vec_init(&keys_sum, 2);
    // for (size_t i = 0; i < 2; i++) {
    //     ECP2_BN254_inf(&(keys_sum.vec[i]));
    // }
    for (size_t k = 0; k < y->size; k++) {
        for (size_t i = 0; i < 2; i++) {
            mpz_t temp = keys_sum.vec[i];
            mpz_add(&(keys_sum.vec[i]),temp, &(key_shares[k].vec[i]));
        }
    }
    ...
    BIG_256_56 y_i,keys_sum_i;
    mpz_t y_i_mod, order,cipher_mpz,keys_sum_mod;
...
//------
    ...
    for (int i = 0; i < 2; i++) {
        ...
        cfe_hash_G1(&h, &label_for_hash);
        mpz_mod(keys_sum_mod,keys_sum.vec[i], order);
        BIG_256_56_from_mpz(keys_sum_i, keys_sum_mod);
        ECP_BN254_mul(&h, &keys_sum_i);
        cfe_string_free(&label_for_hash);
        cfe_string_free(&str_i);
