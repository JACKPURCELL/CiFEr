...
MunitResult test_mcfe_end_to_end(const MunitParameter *params, void *data) {
    ...
    cfe_vec fe_key[num_clients];

    for (size_t i = 0; i < num_clients; i++) {
        cfe_mcfe_encrypt(&(ciphers[i]), &(clients[i]), x.vec[i], label, label_len);
        cfe_mcfe_fe_key_part_init(&(fe_key[i]));
        cfe_mcfe_derive_fe_key_part(&(fe_key[i]), &(clients[i]), &y);
    }

    // decrypt the inner product with the corresponding label
    cfe_error err = cfe_mcfe_decrypt(xy, ciphers, fe_key, label, label_len, &y, bound);
    ...
    for (size_t i = 0; i < num_clients; i++) {
        cfe_mcfe_client_free(&(clients[i]));
...
MunitTest fully_secure_mcfe_tests[] = {
        {(char *) "/end-to-end", test_mcfe_end_to_end, NULL, NULL, MUNIT_TEST_OPTION_NONE, NULL},
        {NULL, NULL,                                      NULL, NULL, MUNIT_TEST_OPTION_NONE, NULL}
};

MunitSuite mcfe_suite = {
        (char *) "/innerprod/fullysec/mcfe", fully_secure_mcfe_tests, NULL, 1, MUNIT_SUITE_OPTION_NONE
};
